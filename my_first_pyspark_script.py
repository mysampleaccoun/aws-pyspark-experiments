...
spark = SparkSession.builder \
    ...
    .getOrCreate()
# read data
df= spark.read.csv("s3://my-pyspark-01/employees.csv",header=True)
df=df.dropna()
...
# Show data
new_df.write.mode("overwrite").option("header", True).csv("s3://my-pyspark-01/output/employee_summary")
avg_salary.write.mode("overwrite").option("header", True).csv("s3://my-pyspark-01/output/department_salary_avg")
count.write.mode("overwrite").option("header", True).csv("s3://my-pyspark-01/output/count")
